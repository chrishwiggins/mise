...
def get_line_count_at_commit(commit_hash):
    ...
    try:
        result = subprocess.run(['git', 'show', f'{commit_hash}:sh/aliases-public.sh'],
...
def main():
    ...
    try:
        ...
        subprocess.run(['git', 'clone', '--filter=blob:none', '--no-checkout',
                       'https://github.com/chrishwiggins/mise.git', repo_dir], check=True)

        os.chdir(repo_dir)

        # Set up sparse checkout for just our file
        subprocess.run(['git', 'sparse-checkout', 'init', '--cone'], check=True)
        subprocess.run(['git', 'sparse-checkout', 'set', 'sh/aliases-public.sh'], check=True)
        ...
        result = subprocess.run(['git', 'log', '--format=%H %ad', '--date=format:%Y',
                                '--follow', '--', 'sh/aliases-public.sh'],
                               capture_output=True, text=True, check=True)
        ...
        avg_lines = [sum(line_counts_by_year[year]) / len(line_counts_by_year[year]) for year in years]
        ...
        plt.plot(years, avg_lines, 'o-', linewidth=2, markersize=8, label='Average Lines', color='blue')

        # Plot max and min as error bars or separate lines
        plt.fill_between(years, min_lines, max_lines, alpha=0.3, color='lightblue', label='Min-Max Range')

        # Formatting
        plt.xlabel('Year', fontsize=12)
        plt.ylabel('Number of Lines', fontsize=12)
        plt.title('Evolution of aliases-public.sh File Size Over Time\n(chrishwiggins/mise repository)', fontsize=14)
        ...
        plt.annotate(f'Started: {avg_lines[0]:.0f} lines',
                     xy=(years[0], avg_lines[0]), xytext=(years[0]+1, avg_lines[0]+50),
                     arrowprops=dict(arrowstyle='->', color='red', alpha=0.7))

        plt.annotate(f'Latest: {avg_lines[-1]:.0f} lines',
                     xy=(years[-1], avg_lines[-1]), xytext=(years[-1]-1, avg_lines[-1]+30),
                     arrowprops=dict(arrowstyle='->', color='red', alpha=0.7))
        ...
        print(f"Growth from {years[0]} to {years[-1]}: {avg_lines[-1] - avg_lines[0]:.1f} lines")
        print(f"Average growth per year: {(avg_lines[-1] - avg_lines[0]) / (years[-1] - years[0]):.1f} lines/year")
